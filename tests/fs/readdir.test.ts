...
Deno.test('readdir returns files and directories', async () => {
...
Deno.test('readdirSync returns files and directories', () => {
...
Deno.test('readdir returns Dirent objects', async () => {
	const dirents = await fs.promises.readdir(testDir, { withFileTypes: true });
	assert(dirents[0] instanceof fs.Dirent);
});

Deno.test('readdirSync returns Dirent objects', () => {
	const dirents = fs.readdirSync(testDir, { withFileTypes: true });
	assert(dirents[0] instanceof fs.Dirent);
});

Deno.test('readdir works without withFileTypes option', async () => {
	const files = await fs.promises.readdir(testDir);
	assert(testFiles.every((entry) => files.includes(entry)));
	assert(testDirectories.every((entry) => files.includes(entry)));
});

Deno.test('readdirSync works without withFileTypes option', () => {
	const files = fs.readdirSync(testDir);
	assert(testFiles.every((entry) => files.includes(entry)));
	assert(testDirectories.every((entry) => files.includes(entry)));
});

Deno.test('Cyrillic file names', () => {
