...
Deno.test('Read a file and check its binary bytes (asynchronous)', async () => {
	const buff = await fs.promises.readFile('elipses.txt');
	assert(((buff[1] << 8) | buff[0]) === 32994);
});

Deno.test('Read a file and check its binary bytes (synchronous)', () => {
...
Deno.test('read file synchronously to non-zero offset', () => {
	const fd = fs.openSync(filepath, 'r');
